...
プレースホルダ設定では、似た名前の項目が並ぶため、以下のように役割を分けて考える。

### プレースホルダ

Excelテンプレート内に記載する置換対象を指す。
...
設定ファイル上では `name` に相当する。テンプレート内では `{{ }}` で囲んで使用し、案件CS生成時に実際の値へ置換される。

### 内部名

Access抽出ファイルやDB由来の列を、アプリ内で扱いやすくするための識別子を指す。
...
設定ファイル上では `source_column` に相当する。これは、画面表示や監査、実装内での参照を安定させるための名前であり、Excelテンプレートに直接書く値ではない。

画面上の表記としては「内部名」よりも「内部キー」または「アプリ内キー」の方が誤解が少ない。MVP画面では `source_column` の表示名を「内部キー」とする。追加・編集フォームでは直接入力させず、プレースホルダ名と装置種別から自動生成して読み取り専用で表示する。

### 値列

Access抽出Excelから実際に値を取得する列名を指す。
...
source_column: command_floating_ip
        ↓
Access抽出ファイル
SBC.xlsx の「コマンド用フローティングIPアドレス」列
        ↓
実際の値
例: 192.168.10.100
```

整理すると、`name` はExcelテンプレートから見える置換キー、`source_column` はアプリ内部で扱うための内部キー、`value_column` はAccess抽出Excelから値を読むための実列名である。
## 値解決の考え方
...
| `source_column` | 自動生成・読み取り専用 | 内部表示や監査用の安定した列名。ユーザーには直接入力させず、`name` と `device_type` から自動生成する。 |
...
プレースホルダ管理機能は、MVPとして一旦ここで区切る。

現時点で以下が実装済みであり、案件CS生成側へ戻って実生成の不足点を確認できる状態になったためである。
...
- 有効化/無効化できる。
- 削除ではなく無効化で運用できる。
- 内部キーはユーザーに直接入力させず、自動生成・読み取り専用にできる。
- Docker環境でも `storage/standard/config/placeholder_mapping.yml` に永続化できる。
- 設計メモに、用語と運用方針を残している。

プレースホルダ管理だけをさらに作り込むよりも、実際の案件CS生成でどの値が不足するか、どの置換が失敗するかを確認した方が、次に追加すべき機能を判断しやすい。

そのため、次の主作業は案件CS生成側へ戻す。

## 後続で優先度が高い改善候補

案件CS生成側へ戻る前提で、プレースホルダ管理側に残しておくとよい改善候補は以下とする。

### 1. プレースホルダ設定の検証強化

参照ファイル、キー列、値列が実際に存在するかをより厳密に確認する。

具体的には以下を確認する。

- `source_file` が Access抽出ファイル配置先に存在するか。
- `key_column` が参照ファイル内に存在するか。
- `value_column` が参照ファイル内に存在するか。
- `enabled=true` に切り替えるとき、案件CS生成で利用可能な定義か。

これは、誤った設定を有効化して案件CS生成時に失敗する事故を防ぐために重要である。

### 2. 値列を手入力ではなく候補選択にする

現在のMVPでは `value_column` は入力できるが、実運用では手入力ミスが起きやすい。

将来的には、`source_file` を選ぶと、そのExcelファイル内の列名を読み取り、`key_column` と `value_column` をプルダウン選択できるようにする。
...
source_file: SBC.xlsx
  -> SBC.xlsx の列名一覧を取得
  -> key_column / value_column の候補として表示
```

これにより、列名の表記ゆれや打ち間違いを減らせる。

### 3. 追加・編集後に生成テストへつなげる

プレースホルダ設定を追加・編集した後、その定義が実際に案件CS生成で解決できるか確認できる導線を検討する。

例:

```text
プレースホルダを追加
  -> 対象の原本・都道府県・ビル・ユニット構成を選択
  -> 値解決を試す
  -> 解決結果またはエラーを表示
```

ただし、この機能は案件CS生成の精度向上と密接に関係するため、単独で作り込むよりも、案件CS生成側の改善と合わせて検討する。
